...
namespace COMMANDS
{
    public class CommandParameters
    {
        ...
        // create this class with a string array of the params, where the flags are whats inside parameters
        public CommandParameters(string[] parameterArray)
        {
            for (int i = 0; i < parameterArray.Length; i++)
            {
                // If a parameter and not a negative value
        ...
        // try to convert to one of the following parameters
        ...
        public bool TryGetValue<T>(string[] parameterNames, out T value, T defaultValue = default(T))
        {
            ...
            // if no identifier was found, search unlabeled ones
            foreach (string parameterValue in unlabeledParameters)
            {
                if (TryCastParameter(parameterValue, out value)) {
                    unlabeledParameters.Remove(parameterValue);
                    return true;
                }
            }
